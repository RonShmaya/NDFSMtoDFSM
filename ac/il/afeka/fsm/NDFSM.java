...
@return a canonic version of this machine. 
	...
	public DFSM toDFSM() throws Exception {
		int id=0;
		Set<State> statesDfsm=new HashSet<>(); //k' sub of p(k)
		Set<State> acceptingStatesDfsm=new HashSet<>(); //A' sub of k'
		StateDfsmFromNdfsm initialStateDfsm=new StateDfsmFromNdfsm(id++); //eps(s)
		Set<Transition> dfsmTransitions=new HashSet<Transition>(); //lamda
		StateDfsmFromNdfsm deadStateDfsm=null; //dead state of the DFSM machine (if neccery)
		LinkedList<StateDfsmFromNdfsm> statesForCheck=new LinkedList<StateDfsmFromNdfsm>();//states we need to find the transitions
		StateDfsmFromNdfsm currentState; //the state we check now

		//get eps(state) for the first new state
		initialStateDfsm.addAllGroup(eps(this.initialState,null));
		statesDfsm.add(initialStateDfsm);
		checkIfAcceptingState(initialStateDfsm,acceptingStatesDfsm);
		statesForCheck.add(initialStateDfsm);
		
		while(!statesForCheck.isEmpty()) {
			currentState=statesForCheck.pollFirst();
			for (Character symbol : alphabet) {
				StateDfsmFromNdfsm newState=getNextGroupState(currentState, symbol); //get the new state
				if (!newState.isGroupEmpty()) {
					newState.setId(id++);
					dfsmTransitions.add(new Transition(currentState, symbol, newState));
					if(!statesDfsm.contains(newState)) {
						statesForCheck.add(newState);
						statesDfsm.add(newState);
						checkIfAcceptingState(newState,acceptingStatesDfsm);
					}
				}
				else {
					if(deadStateDfsm==null) { 
						deadStateDfsm=new StateDfsmFromNdfsm(id++);
						statesDfsm.add(deadStateDfsm);
						addDeadStateTranstion(deadStateDfsm,dfsmTransitions);
					}
					dfsmTransitions.add(new Transition(currentState, symbol, deadStateDfsm));
				}
			}
		}
		return new DFSM(statesDfsm, this.alphabet, dfsmTransitions, initialStateDfsm, acceptingStatesDfsm);
	}

	private Set<State> eps(State state,Set<State> epsForState){
		if(epsForState==null) 
			epsForState=new HashSet<State>();
		
		epsForState.add(state);
		for (Transition t : this.transitions.transitions()) 
			if(t.symbol()==Alphabet.EPSILON && t.fromState()==state && !epsForState.contains(t.toState())) 
				eps(t.toState(),epsForState);

		return epsForState;
	}
	private StateDfsmFromNdfsm getNextGroupState(StateDfsmFromNdfsm state,Character symbol){
		StateDfsmFromNdfsm newState=new StateDfsmFromNdfsm();
		for (State s : state.getGroup()) {
			for (Transition t : this.transitions.transitions()) 
				if(t.symbol()==symbol && t.fromState()==s)
					newState.addAllGroup(eps(t.toState(),null));
		}
		return newState;
	}
	private void checkIfAcceptingState(StateDfsmFromNdfsm state,Set<State> acceptingStatesDfsm) {
		for (State s : state.getGroup()) {
			if(this.acceptingStates.contains(s)) {
				acceptingStatesDfsm.add(state);
				return;
			}
		}
		
	}
	private void addDeadStateTranstion(StateDfsmFromNdfsm deadStateDfsm,Set<Transition> dfsmTransitions) {
		for (Character symbol : this.alphabet) 
			dfsmTransitions.add(new Transition(deadStateDfsm, symbol, deadStateDfsm));
	}
